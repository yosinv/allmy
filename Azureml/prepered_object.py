class QuitoClassifier(object):
    ...
    def __init__(self, threshold):
        ...
        # features
        self.metadata_cols = ['Claim_Nr', 'Client_Claim', ]  # Endors_Nr - questionable
        self.metadata_cols = ['Client_Claim', ]
        self.ordinal_feat_lst = list(
            set(['Claim_Date_month_year', 'Claim_Date_day', 'Claim_Date_month', 'Claim_Date_year']))
        self.cat_feat_lst = list(set(
            ['Claim_Place_Code', 'Claim_Place_Zone_Code', 'Fualt_Insured_Code', 'Vehicle_Towed', 'Police_arrive',
             'Photos_taken', 'Damage_Dsc_array']))
        self.feat_to_clipp = list(set(['Claim_Place_Zone_Code', ]))
        self.decomp_by_range = list(set(['Claim_Date_month_year']))

        self.all_features = list(set(
            self.ordinal_feat_lst + self.cat_feat_lst + self.feat_to_clipp + self.decomp_by_range + self.metadata_cols))

        self.ORDINAL_COLS = tuple(self.ordinal_feat_lst + self.decomp_by_range)
        self.CATEGORICAL_COLS = tuple(self.cat_feat_lst + self.feat_to_clipp)
        self.NUMERICAL_COLS = ()

        # encoders+model paths:
        #         self.model_date = model_date
        #         self.base_dir = '{}_{}'.format(base_dir, model_date)

        dtypes_dct_path = Model.get_model_path(model_name='dtypes_dct_claim_feat')
        labelEncoders_path = Model.get_model_path(model_name='ordinal_encoders_claim_feat')
        ...
        # DATA
        dataset1 = Dataset.get_by_name(workspace, name='client_df_for_real_time')
        #         client_df_rt= dataset1.to_pandas_dataframe()

        dataset2 = Dataset.get_by_name(workspace, name='clipping_values_df_claim_feat')
        #         clipped_vals_df = dataset2.to_pandas_dataframe()

        dataset3 = Dataset.get_by_name(workspace, name='bins_df_claim_feat')
        #         bins_df = dataset3.to_pandas_dataframe()

        self.bins_df = dataset3.to_pandas_dataframe()
        self.clipped_vals_df = dataset2.to_pandas_dataframe()

        print('AZURE ML SUB MODEL "dtypes_dct_path" : ', dtypes_dct_path)
        self.dtypes_dct = pickle.load(open(dtypes_dct_path, 'rb'))

        print('AZURE ML SUB MODEL "ordinalEncoders_path" : ', ordinalEncoders_path)
        self.ordinalEncoders = pickle.load(open(ordinalEncoders_path, 'rb'))

        print('AZURE ML SUB MODEL "labelEncoders_path" : ', labelEncoders_path)
        ...
        print('AZURE ML SUB MODEL "rel_feat_feature_importance_path_client" : ',
              rel_feat_feature_importance_path_client)
        self.rel_feat_feature_importance_client = pickle.load(open(rel_feat_feature_importance_path_client, 'rb'))

        self.client_df_rt = dataset1.to_pandas_dataframe()
        self.client_df_rt.drop_duplicates('CLIENT_CLAIM', inplace=True)

        #         VALIDATE DATA
        print('client_df_rt: ', client_df_rt.iloc[:3])
        print('clipped_vals_df: ', clipped_vals_df.iloc[:3])
        print('bins_df: ', bins_df.iloc[:3])
    ...
    def query(self, json_obj, mode='xgb'):
        ...
        try:
            ...
            print('step: {} time running: {} time finish: {}'.format(step, time_running, dt.now()))

        except Exception as e:
            print('Failed to: step: {}, Reason: {}'.format(step, str(e)))
        ...
        try:
            ...
            for col in self.dtypes_dct.keys():
                try:
                    ...
                    print('step: {} col: {} error: {}'.format(step, col, e))
    ...
    def _save_data(self):
        ...
        try:
            ...
            #             DATA!!!!! CHANGED from BASED DIR - all this current function is not releveant!!!
            client_df_path = '{}/client_df_for_real_time.csv'.format(os.getcwd())
