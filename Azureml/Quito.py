...
class QuitoClassifier(object):
    def __init__(self, base_dir, model_date, threshold):
        ...
        bins_path = os.path.join(self.base_dir, 'bins_df_claim_feat.csv')
        clipped_dct_path = os.path.join(self.base_dir, 'clipping_values_df_claim_feat.csv')
    ...
    def query(self, json_obj, mode='xgb'):
        ...
        try:
            ...
            client_df = client_df.merge(self.client_df_rt, left_on=['Claim_Nr', 'Client_Claim'],
                                        right_on=['Claim_Nr', 'CLIENT_CLAIM'])
            ...
            for col in list(client_df.select_dtypes(include=[np.float]).columns):
            ...
            # predict and save:
            self.proba_df = self._predict(df[self.metadata_cols], df.drop(self.metadata_cols, axis=1), mode)
            # self._save_data()
    ...
    def _produce_new_features(self, X):
        ...
        try:
            ...
            pca_cols = ['clipped_decomp_pca_feat_{}'.format(x + 1) for x in range(self.pca.n_components)]
            ...
            poly_df.rename(columns={col: ('clipped_decomp_pol_feat_{}'.format(col)) for col in poly_df.columns},
                           inplace=True)
            ...
            X_anom_scor['clipped_decomp_anomaly_score_feat'] = self.scaler_after_score.transform(
    ...
    def _predict(self, client_metadata, values_to_predict, mode):
        ...
        try:
            if (mode == 'ensamble'):
                model_probas = np.zeros((len(values_to_predict), 1))
                model_probas[:, 0] = self.rfc_model.predict_proba(values_to_predict)[:, 1]
                model_probas[:, 1] = self.xgb_model.predict_proba(values_to_predict)[:, 1]
                model_probas[:, 2] = self.lgb_model.predict(values_to_predict)
                ensemble_probas = model_probas.mean(axis=1)
                # ensemble_probas = self.lgb_model.predict(values_to_predict)
                probas_df = pd.DataFrame(ensemble_probas, columns=['proba'])
            elif (mode == 'lgb'):
                model_proba = self.lgb_model.predict(values_to_predict)
                probas_df = pd.DataFrame(model_proba, columns=['proba'])
            '''
            def f(x):
                if (x >= self.ts):
                  return 1
                elif (x <= 0.5 - (self.ts - 0.5)):
                  return 0
                else:
                  return 2
            '''
            ...
            probas_df['Lima_prediction'] = probas_df['Lima_prediction_id'].apply(lambda x: self.prediction_types[x])
